...
const register = catchAsync(async (req, res) => {
  ...
  let authToken = await tokenService.generateAuthTokens(newUser);
  return res.status(httpStatus.CREATED).json({user:newUser,tokens:authToken});
});
...
const login = catchAsync(async (req, res) => {
  ...
  let authToken = await tokenService.generateAuthTokens(user);
  return res.status(httpStatus.OK).json({user:user,tokens:authToken});
});
